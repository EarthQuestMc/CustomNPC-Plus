...
import noppes.npcs.client.EQSkinManager;
...
public class RenderNPCInterface extends RenderLiving {
    ...
    public ResourceLocation getEntityTexture(Entity entity) {
        ...
        if (npc.textureLocation == null) {
            ...
            /* ==========================================================
               EARTHQUEST — PRIORITÉ ABSOLUE POUR skinType == PLAYER
               ========================================================== */
            if (npc.display.skinType == 1 && npc.display.playerProfile != null) {
                try {
                    String username = npc.display.playerProfile.getName();
                    if (username != null && !username.isEmpty()) {
                        ResourceLocation eq = EQSkinManager.get(username);
                        if (eq != null) {
                            npc.textureLocation = eq;
                            LastTextureTick = 0;
            ...
            if (npc.display.skinType == 0) {
                ...
                Map map = mc.func_152342_ad().func_152788_a(npc.display.playerProfile);
                if (map.containsKey(Type.SKIN)) {
                    npc.textureLocation = mc.func_152342_ad()
                ...
                if (npc.display.url.isEmpty()) {
                    ...
                    try {
                        ResourceLocation loc = ClientCacheHandler
                            ...
                            .getLocation();
                        npc.textureLocation = (loc != null) ? loc : fallBackSkin(npc);
                    } catch (Exception ignored) {
                    }
                } else if (LastTextureTick < 5) {
                    return fallBackSkin(npc);
                } else {
                    try {
                        ResourceLocation loc = ClientCacheHandler
    ...
    private ResourceLocation fallBackSkin(EntityNPCInterface npc) {
        switch (npc.display.modelType) {
            case 2:
                return alex;
            case 1:
                return steve64;
    ...
    private ResourceLocation adjustLocalTexture(EntityNPCInterface npc, ResourceLocation location) throws IOException {
        ...
        try {
            ...
            ImageDownloadAlt object = new ImageDownloadAlt(null, npc.display.texture,
                SkinManager.field_152793_a, new ImageBufferDownloadAlt(true));
            ...
            location = new ResourceLocation(
                npc.display.modelType == 0 ? "skin/" + sb : "skin64/" + sb
            );
